...
export default function CourseAdminPage() {
  ...
  const fetchReferralStats = async () => {
    ...
    try {
      const { data } = await authService.getReferralStats();
      setRefStats(data);
    } catch {
  ...
  const fetchWithdrawals = async () => {
    ...
    try {
      const { data } = await authService.getAdminWithdrawals();
  ...
  const handleUpdateWithdrawalStatus = async (id: string, newStatus: 'confirmed' | 'rejected') => {
    try {
      await authService.updateAdminWithdrawal(id, { status: newStatus });
      toast.success(`Withdrawal request marked as ${newStatus}.`);
      fetchWithdrawals();
      fetchReferralStats();
    } catch (err: any) {
      toast.error(err?.response?.data?.detail || 'Failed to update withdrawal status.');
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 pb-24">
      ...
      <div className="mb-8 p-6 rounded-2xl bg-dark-900 border border-dark-800 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-xl relative overflow-hidden">
        ...
        <div className="flex bg-dark-950 border border-dark-850 p-1.5 rounded-xl shrink-0">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-semibold tracking-wide transition-all flex items-center gap-1.5 ${
              activeTab === 'referrals' ? 'bg-neb-600 text-white shadow-md' : 'text-gray-400 hover:text-white'
            }`}
          >
      ...
      <div className="animate-in fade-in duration-300">
        ...
        {activeTab === 'referrals' && (
          <div className="flex flex-col gap-6">
            <div>
              <h2 className="text-xl font-bold text-white">Referral Program Statistics</h2>
              <p className="text-xs text-gray-400">Monitor total distributed rewards, pending withdrawals, and active referral balances.</p>
            </div>
            ...
            {refStatsLoading ? (
              ...
              <div className="flex flex-col items-center justify-center text-center p-12 bg-dark-900/20 border border-dark-800 rounded-2xl">
                <AlertCircle className="h-12 w-12 text-dark-700 mb-3" />
                <h3 className="text-base font-bold text-white mb-1">Failed to load stats</h3>
                <Button onClick={fetchReferralStats} size="sm">Retry</Button>
              </div>
            ) : (
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {/* Total Distributed Card */}
                  <div className="card p-6 bg-dark-900/40 border-dark-800 flex flex-col justify-between">
                    <div>
                      <span className="text-[10px] uppercase font-bold tracking-widest text-neb-450">Total Rewards Distributed</span>
                      <h3 className="text-3xl font-extrabold text-white mt-2">{(refStats.total_given_out || 0).toLocaleString()} FRS</h3>
                      <p className="text-xs text-gray-500 mt-2">Cumulative sum of all rewards generated by user subscription referrals.</p>
                    </div>
                  </div>
                  ...
                  <div className="card p-6 bg-dark-900/40 border-dark-800 flex flex-col justify-between">
                    <div>
                      <span className="text-[10px] uppercase font-bold tracking-widest text-amber-450">Remaining User Balances</span>
                      <h3 className="text-3xl font-extrabold text-white mt-2">{(refStats.total_balance_remaining || 0).toLocaleString()} FRS</h3>
                      <p className="text-xs text-gray-500 mt-2">Total cash currently sitting in user accounts eligible for future withdrawals.</p>
                    </div>
                  </div>
                  ...
                  <div className="card p-6 bg-dark-900/40 border-dark-800 flex flex-col justify-between">
                    <div>
                      <span className="text-[10px] uppercase font-bold tracking-widest text-indigo-400">Pending Withdrawals</span>
                      <h3 className="text-3xl font-extrabold text-white mt-2">{(refStats.total_pending || 0).toLocaleString()} FRS</h3>
                      <p className="text-xs text-gray-500 mt-2">Funds requested by users that are currently awaiting admin approval.</p>
                    </div>
                  </div>

                  {/* Confirmed Payouts Card */}
                  <div className="card p-6 bg-dark-900/40 border-dark-800 flex flex-col justify-between">
                    <div>
                      <span className="text-[10px] uppercase font-bold tracking-widest text-emerald-400">Paid Out Withdrawals</span>
                      <h3 className="text-3xl font-extrabold text-white mt-2">{(refStats.total_withdrawn || 0).toLocaleString()} FRS</h3>
                      <p className="text-xs text-gray-500 mt-2">Total referral rewards already transferred and confirmed via Mobile Money.</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-4 mt-4">
                  <div>
                    ...
                    <p className="text-xs text-gray-400">Approve or reject Mobile Money withdrawal requests submitted by referrers.</p>
                  </div>

                  {withdrawalsLoading ? (
                    <div className="flex items-center justify-center py-12 bg-dark-900/30 border border-dark-800 rounded-2xl">
                      <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-neb-500"></div>
                    </div>
                  ) : withdrawals.length === 0 ? (
                    <div className="flex flex-col items-center justify-center text-center p-12 bg-dark-900/20 border border-dark-800 rounded-2xl">
                      <AlertCircle className="h-10 w-10 text-dark-700 mb-2" />
                      <h4 className="text-sm font-bold text-white mb-1">No withdrawal requests found</h4>
                      <p className="text-xs text-gray-450">Users haven't requested any withdrawals yet.</p>
                    </div>
                  ) : (
                    <div className="card bg-dark-900/20 border border-dark-800 overflow-hidden shadow-xl">
                      <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse text-xs">
                          ...
                          <tbody className="divide-y divide-dark-850">
                            {withdrawals.map((w: any) => (
                              <tr key={w.id} className="hover:bg-dark-900/30 transition">
                                <td className="p-4 font-semibold text-white">
                                  {w.user_email || 'Unknown User'}
                                </td>
                                <td className="p-4 text-gray-350">{w.momo_name}</td>
                                <td className="p-4 font-mono text-gray-450">{w.momo_number}</td>
                                  ...
                                  <span
                                    className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                                      w.status === 'confirmed'
                                        ...
                                        : 'bg-amber-950/50 border border-amber-800/40 text-amber-450'
                                    }`}
                                  >
                                    {w.status}
                                  </span>
                                ...
                                <td className="p-4 text-center">
                                  {w.status === 'pending' ? (
                                    <div className="flex items-center justify-center gap-2">
                                      <button
                                        onClick={() => handleUpdateWithdrawalStatus(w.id, 'confirmed')}
                                        className="p-1 rounded bg-emerald-950/30 border border-emerald-800/50 hover:bg-emerald-800 hover:text-white text-emerald-450 transition-all"
                                      ...
                                      <button
                                        ...
                                        className="p-1 rounded bg-red-950/30 border border-red-800/50 hover:bg-red-800 hover:text-white text-red-400 transition-all"
